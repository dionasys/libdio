...
filesToMerge = ['src/required.lua', 'src/node.lua', 'src/coordinator.lua', 'src/utilities.lua', 'src/pss.lua', 'src/tman.lua', 'src/ranking.lua' ]
stdOutput = 'libdio.lua' #std output name used if no out=yyy option is passed
stdInput  = 'examples/myExample1.lua' #this file is the file usually used for tests (it contains many diff. tests depending on the content of myTest.lua), for a user specific app the option in=xxx must be used to include the source for the specific app.
