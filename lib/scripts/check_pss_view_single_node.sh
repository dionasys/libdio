...
> $OUTPUT_DATA_DIR/$JOB/$OUTPUT_BASE_FILE
  ...
  for cycle in `seq 1 1 300`; do 
    echo "$1 $2 $cycle"
    > $OUTPUT_DIR/$OUTOPUT_BASE_FILE 
    cat $INPUT_DATA_DIR/$JOB  | grep "node: $NODE " | grep "cycle: $cycle " | grep "CURRENT PSS_VIEW" | tail -1 | awk '{$1=$3=$4=$5=$6=$7=$10=$11=$12=""; print $0 }'  >  $OUTPUT_DATA_DIR/$JOB/$OUTPUT_BASE_FILE; 
  done
echo "end."
