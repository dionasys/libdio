...
Coordinator.launch=function(node, running_time, delay)
	...
	-- this is the event for a msg sender, it will be 
	log:print("[Coordinator.launch] - at node: "..job.position.." Coordinator.pigbackMsg : "..tostring(Coordinator.pigbackMsg))
	if Coordinator.pigbackMsg then
		log:print("[Coordinator.launch] - at node: "..job.position.." scheduling a periodic task ")
		events.periodic(3, function() Coordinator:sender() end)
		events.periodic(3, function() log:print("running my periodic thread") end)
...
Coordinator.sender=function()
	...
	-- peer3={ip = "127.0.0.1", port = 400}
		log:print(" DEBUG - [Coordinator.sender] - COORDINATOR SENDER at node: "..job.position.." RUNNING.")
	
		Coordinator.sender_msgs_lock:lock()
			if #Coordinator.senderBuffer > 0 then
					log:print(" DEBUG - [Coordinator.sender] - COORDINATOR SENDER at node: "..job.position.." number of msgs in the senders buffer: "..#Coordinator.senderBuffer)
...
Coordinator.send=function(algoId, dst, buf, eventToFire)
	...
	-- peer2={ip = "127.0.0.1", port = 300}
	-- peer3={ip = "127.0.0.1", port = 400}
		log:print(" DEBUG - [Coordinator.SEND] - at node: "..job.position.." - COORDINATOR SEND method invoked at node: "..job.position)
		local algo = nil
		for k,v in pairs(Coordinator.algos) do 
				if v.id==algoId then
					algo = v.obj
		...
		if algo then
			...
			if Coordinator.pigbackMsg then
				log:print(" DEBUG - [Coordinator.SEND] -  at node: "..job.position.." COORDINATOR SEND - Coordinator.pigbackMsg active")
				...
				log:print(" DEBUG - [Coordinator.SEND] -  at node: "..job.position.." COORDINATOR SEND - Coordinator.pigbackMsg NOT active at node: "..job.position)
